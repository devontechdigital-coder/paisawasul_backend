...
export const AddAdminProduct = async (req, res) => {
  try {
    const {
      ...
      tag,
      features,
      specifications, gst, weight, hsn, sku, canonical,userId,minPrice,day,protype
    } = req.body;
    ...
    const newProduct = new productModel({
      ...
      tag,
      features,
      specifications: updatespecifications, gst, weight, hsn, sku, canonical,userId,minPrice,day, 
      protype
    });
...
export const updateProductAdmin = async (req, res) => {
  try {
    ...
    const {
      ...
      what_is_included, 
      what_is_excluded, 
      extra_content ,minPrice,day,protype
    } = req.body;
...
export const editHomeData = async (req, res) => {
  try {
    ...
    const {
      ...
      startTime,
      endTime,
      keyId,keySecret,recommended_user,gst,startNightTime,endingNightTime,nightCharges,ios,android
    } = req.body;

    console.log('recommended_userrecommended_userrecommended_user',recommended_user)
    ...
    let updateFields = {
      ...
      noOfDays,
      startTime,
      endTime, keyId,keySecret,recommended_user,gst,startNightTime,endingNightTime,nightCharges,ios,android
    };
...
export const editHomeLayoutData = async (req, res) => {
  try {
    ...
    const {
      ...
      service_logos,
      service_banner_images, slider_img, top_bar,home_content,
...
export const EmployeeTimerOrderController_old = async (req, res) => {
  const { OrderId, time ,otp } = req.body;
  if(!otp){
  ...
  if (!["start", "end"].includes(time)) {
    return res.status(400).json({
      message: "Invalid time type. Must be 'start' or 'end'.",
      success: false,
    });
  }
   ...
   if(otp !== Order.OTP){
    ...
    if (time === "start") {
      ...
      if (Order.startTime) {
        ...
        const basePrice = parseFloat(item.price || 0); // e.g., 519
        const perMinuteRate = parseFloat(item.hour || 0); // e.g., 4
        const extraMinutes = diffInMinutes > 60 ? diffInMinutes - 60 : 0;

        const extraCharge = perMinuteRate * extraMinutes;
        const finalPriceBeforeGst = basePrice + extraCharge;

        const gst = (finalPriceBeforeGst * 18) / 100;
        const totalAmount = finalPriceBeforeGst + gst;

        // Update item values
        item.regularPrice = Math.round(finalPriceBeforeGst); // Keep base
        item.price = Math.round(finalPriceBeforeGst); // Update actual price
        // item.gst = Math.round(gst);
        item.gst = 18;

        // Update order total
        Order.totalAmount = Math.round(totalAmount);

        // 🔥 Force Mongoose to track changes in nested object
    ...
    return res.status(200).json({
      message: `Job timer ${time}ed successfully`,
      success: true,
      [time + "Time"]: currentTime,
      updatedItem: Order.items[0],
...
export const EmployeeTimerOrderController_old_last = async (req, res) => {
  const { OrderId, time, otp } = req.body;
  ...
  try {
    ...
    // If OTP doesn't exist on order, generate and save one
    if (!order.OTP) {
      const generatedOtp = Math.floor(1000 + Math.random() * 9000);
      order.OTP = generatedOtp;
      await order.save();
      return res.status(400).json({ message: "OTP not found. New OTP generated.", success: false, otp: generatedOtp });
    }
    ...
    if (time === "start") {
      ...
      if (order.startTime) {
        ...
        if (item) {
          ...
          const perMinuteRate = parseFloat(item.hour || 0);
          const extraMinutes = diffInMinutes > 60 ? diffInMinutes - 60 : 0;
          ...
          item.regularPrice = Math.round(finalPriceBeforeGst);
          item.price = Math.round(finalPriceBeforeGst);
          item.gst = 18;

          order.totalAmount = Math.round(totalAmount);

          // Mark items as modified so Mongoose knows to update it
          order.markModified("items");
        }
      }
    }
...
export const completeOrderUser_admin_old = async (id,res) => {
  try {
...
export const completeOrderUser_admin = async (id) => {
  try {
...
const home = await homeModel.find();
  const amount = Math.round((Order.totalAmount * home[0].commission) / 100);
      ...
      if (user.wallet === amount) {
             ...
             return { success: false, message: "You do not have enough funds to accept this Order" };
      }
    }
...
const finalPrice = amount + gstAmount;
  ...
  //  Order.payment = 1;
    ...
    transaction.save(),
     ...
     return { success: false, message: `Error while updating Order: ${error.message}` };
  }
};


export const EmployeeTimerOrderController_old_11_nov  = async (req, res) => {
  const { OrderId, time, otp } = req.body;
    console.log(OrderId, time, otp)
  ...
  try {
    ...
    // If OTP doesn't exist on order, generate and save one
    if (!order.OTP && time === "start") {
      const generatedOtp = Math.floor(1000 + Math.random() * 9000);
      order.OTP = generatedOtp;
      await order.save();
      return res.status(400).json({
        message: "OTP not found. New OTP generated.",
        success: false,
        otp: generatedOtp,
      });
    }
    ...
    // Check if OTP matches
    if (parseInt(otp) !== parseInt(order.OTP) && time === "start") {
      return res.status(400).json({ message: "Incorrect OTP!", success: false });
    }
    ...
    if (time === "start") {
      ...
      if (order.startTime) {
        ...
        const diffInMs = end - start;
        const diffInMinutes = Math.floor(diffInMs / (1000 * 60));
        const selectedHour = diffInMinutes / 60;
        ...
        // if (item) {
        //   const Product = item;
        //   const ProductsSale = parseFloat(Product.salePrice || 0); // Use default or provided sale price
        //   const NightCharges = parseFloat(Product.nightCharges || 0); // Can be 0 if not applicable
        //   const minPrice = parseFloat(Product.minPrice || 0);
        //   const dayPrice = parseFloat(Product.fullday || 0);
        ...
        //       myprice = dayPrice;
        //     } else {
        //       myprice = (selectedHour / 24) * dayPrice;
        //     }
        //   } else {
        //     if (selectedHour <= 0.5) {
        //       myprice = ProductsSale;
        //     } else {
        //       const extraMinutes = selectedHour * 60 - 30;
        //       myprice = ProductsSale + (extraMinutes * minPrice);
        //     }
        //   }

        //   const totalBeforeGst = myprice + NightCharges;
        //   const totalWithGst = Math.round(totalBeforeGst * 1.18); // 18% GST

        //   // Update item
        //   item.price = Math.round(myprice);
        //   item.GSTPrice = totalWithGst;
        //   item.NightCharges = NightCharges;
        //   item.gst = 18;

        //   order.totalAmount = totalWithGst;
        //   order.markModified("items");
...
if (item) {
  const Product = item;
  const ProductsSale = parseFloat(Product.salePrice || 0);
  const NightCharges = parseFloat(Product.nightCharges || 0);
  const minPrice = parseFloat(Product.minPrice || 0);
  const dayPrice = parseFloat(Product.fullday || 0);
  ...
  const totalAmount = Math.round(myprice + NightCharges);

  // Update item
  item.price = Math.round(myprice);
  item.GSTPrice = totalAmount; // Since GST is 0, it's equal to total
  item.NightCharges = NightCharges;
  item.gst = 0;
  ...
  order.markModified("items");
   ...
   await completeOrderUser_admin(OrderId,res);
    }

    // Generate new OTP for future operation
    const generatedOtp = Math.floor(1000 + Math.random() * 9000);
    order.OTP = generatedOtp;
...
export const EmployeeTimerOrderController_new_11_nov = async (req, res) => {
  const { OrderId, time, otp } = req.body;
  console.log(OrderId, time, otp);
    ...
    if (time === "start") {
      ...
      if (order.startTime) {
        ...
        if (item) {
          ...
          let myprice;
          if (dayPrice && dayPrice !== 0) {
            myprice = selectedHour <= 24 ? dayPrice : (selectedHour / 24) * dayPrice;
          } else {
            myprice = selectedHour <= 0.5
              ? ProductsSale
              : ProductsSale + ((selectedHour * 60 - 30) * minPrice);
          }

          const totalAmount = Math.round(myprice + NightCharges);
          item.price = Math.round(myprice);
          item.GSTPrice = totalAmount;
          item.NightCharges = NightCharges;
          item.gst = 0;
          order.totalAmount = totalAmount;
          order.markModified("items");
        ...
        // Call helper but DO NOT send response from it
        await completeOrderUser_admin(OrderId); 
      }
    }

    // Generate a new OTP for next operation
    const generatedOtp = Math.floor(1000 + Math.random() * 9000);
    order.OTP = generatedOtp;
    await order.save();
...
export const EmployeeTimerOrderController_new_20_jan_26  = async (req, res) => {
  const { OrderId, time, otp } = req.body;
    console.log(OrderId, time, otp)
...
if (item) {
  ...
  order.markModified("items");
      ...
      const result = await completeOrderUser_admin(OrderId);
...
export const EmployeeTimerOrderController  = async (req, res) => {
  const { OrderId, time, otp } = req.body;
    console.log(OrderId, time, otp)
...
const newItemsTotal = order.items
  .filter(item => Number(item.addOns) === 1)
  .reduce((sum, item) => {
  ...
  order.totalAmount = totalAmount+newItemsTotal;
  order.markModified("items");
}
...
export const GetUserTimerOrdersController = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: "Orders with active timers fetched successfully",
      success: true,
      data: orders,
    });
  } catch (error) {
...
export const getAllOrderAdmin = async (req, res) => {
 ...
 const parsedStatus = statusFilter ? statusFilter.split(',').map(Number) : null;


    // Add status filter to the query if statusFilter is provided
    if (statusFilter) {
      query.status = { $in: parsedStatus }; // Use $in operator to match any of the values in the array
    }
 // Add status filter to the query if statusFilter is provided
    if (statusNoFilter) {
...
if (job === 'All') {
  query.agentId = { $in: [null, undefined] }; // Matches null or missing
    ...
    // Retrieve the Razorpay keys from the database
          ...
          const { recommended_user} = homeData;

          if(recommended_user && recommended_user.length !== 0){
            const isIncluded = recommended_user.includes(employeeId);
if(!isIncluded){
  query.agentId = { $in: 2 }; // Matches null or missing
...
export const AdminGetAllEmployee = async (req, res) => { 
  try {
    ...
    // Fetch users based on the filter
    const users = await userModel.find(filter, '_id username department longitude latitude online');
...
export const editOrderAdmin = async (req, res) => {
  try {
    ...
    if(status === 100){
      const updatedOrder = await orderModel.findByIdAndUpdate(id, updatepayFields, {
      new: true,
    });
    }else{
...
export const UserAcceptRequest = async (req, res) => {
  try {
    const { OrderId, Order } = req.body;

    if (!OrderId || !Order || !Array.isArray(Order)) {
      return res.status(400).json({
        success: false,
    ...
    // Calculate price of new items
    const newItemsTotal = Order.reduce((sum, item) => {
      const price = Number(item.price) || 0;
      const qty = Number(item.quantity) || 1;
      return sum + price * qty;
    }, 0);
    ...
    // Update total price
    order.totalAmount = (order.totalAmount || 0) + newItemsTotal;
...
export const getAllUserAdmin = async (req, res) => {
  try {
    ...
    const limit = parseInt(req.query.limit) || 10; // Number of documents per page, default is 10
    const searchTerm = req.query.search || ""; // Get search term from the query parameters
    const type = req.query.type || null; // Get search term from the query parameters
    const status = req.query.status || null; // Get search term from the query parameters
...
export const UserloginAll = async (req, res) => {
  try {
    const { email, password, latitude, longitude ,fcm} = req.body;
...
export const getAllOrdersBillingAdmin = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1; // Current page, default is 1
    const limit = parseInt(req.query.limit) || 10; // Number of documents per page, default is 10
    const searchTerm = req.query.search || ""; // Get search term from the query parameters
    ...
    const totalData = await orderModel.countDocuments(query); // Count total documents matching the query
...
export const getAllPaymentsInvoiceAdmin = async (req, res) => {
 ...
 const fullData = await paymentModel.find(query).lean(); // no skip/limit
const totalAmountSum = fullData.reduce((sum, item) => sum + (item.totalAmount || 0), 0);
...
const commission = home[0].commission;
    ...
    return res.status(200).send({
      ...
      success: true,
      data:  data , // Return users array
      totalAmountSum, // ✅ this is for all matching data (not just page)
      commission,
      totalAmt: totalAmountSum,
      Amt: totalAmountSum - (totalAmountSum * 0.18).toFixed(2),
      Fees: Number(((totalAmountSum - (totalAmountSum * 0.18)) * (commission / 100)).toFixed(2)),
      Unpaid: (totalAmountSum - (totalAmountSum * 0.18).toFixed(2) ) - (( totalAmountSum - (totalAmountSum * 0.18).toFixed(2) ) * (commission / 100 ).toFixed(2)) ,
      Paid: 0
    });
  } catch (error) {
    return res.status(500).send({
...
// for order payment invoices

export const getAllOrderPaymentsInvoiceAdmin = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1; // Current page, default is 1
    const limit = parseInt(req.query.limit) || 10; // Number of documents per page, default is 10
    const searchTerm = req.query.search || ""; // Get search term from the query parameters
...
const withdrawals = await withdrawalModel
  .find({ userId: userId, status: 0 })
  .populate('userId');
const Tamount = withdrawals.reduce((sum, item) => sum + (item.amount || 0), 0);
TotalOutstand = Math.round(Tamount);
    }else{
      const withdrawals = await withdrawalModel
  .find({status: 0 })
  .populate('userId');
...
TotalOutstand =  Math.round(Tamount);
    ...
    const totalData = await orderModel.countDocuments(query); // Count total documents matching the query
 const fullData = await orderModel.find(query).lean(); // no skip/limit
const totalAmountSum = fullData.reduce((sum, item) => sum + (item.totalAmount || 0), 0);
  const amountCom = Math.round(( totalAmountSum * home[0].commission) / 100);

    const amountwithoutGSt = Math.round(( amountCom * 18) / 100);
  const amountWithGst = amountCom + amountwithoutGSt ;
   const TotalGst = amountwithoutGSt;

const HelperEarning =  totalAmountSum - amountWithGst;
const TotalITFees = amountCom;
    ...
    const data = await orderModel
      ...
      .populate({
        ...
        path: "agentId", // The field to populate
        select: "phone username statename gstin",
      })
    ...
    return res.status(200).send({
      ...
      data:  data , // Return users array
      totalAmountSum, // ✅ this is for all matching data (not just page)
      extraData : { totalAmountSum, HelperEarning,
      TotalGst,
      TotalITFees,
      TotalOutstand},
      commission
